...
static int main_handler(int event_type, int param_one, int param_two)
{
  if (EVT_INIT == event_type)
  {
    ...
    DIR *dir;
    struct dirent *ent;
    if ((dir = opendir("/mnt/ext1/My pictures/PocketFrame")) != NULL)
    {
      // Print all the files and directories within directory
      while ((ent = readdir(dir)) != NULL)
      {
        char picfile[300];
        strcpy(picfile, "/mnt/ext1/My pictures/PocketFrame/");
        ...
        if (access(picfile, F_OK) == 0)
        {
          ...
          {
            ibitmap *picture = LoadJPEG(picfile, ScreenWidth(), ScreenHeight(), 100, 100, 1);
            Stretch(picture->data, IMAGE_GRAY2, picture->width, picture->height, picture->scanline, 0, 0, picture->width, picture->height, 0);
            log_message(picfile);
